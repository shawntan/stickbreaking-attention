...
def create_compute_tile(scale, inv_log2, dim, block_M, block_N, is_causal, accum_dtype):
    @T.macro
    def compute_tile(
            bx, k,
...
def flashattn_fwd(batch, heads, seq_len, dim, is_causal, block_M, block_N,
    ...
    accum_dtype = "float"
    compute_tile = create_compute_tile(scale, inv_log2, dim, block_M, block_N, is_causal, accum_dtype)
    @T.prim_func
    def flash_fwd(
