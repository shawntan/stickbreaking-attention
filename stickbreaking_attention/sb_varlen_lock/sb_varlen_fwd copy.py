...
def compute_block(
    ...
    log_om_beta = -softplus(p, is_compiling=is_compiling)
    log_om_beta_ = log_om_beta.to(q.dtype)
    if on_band:  # diagonal
        if attend_current:
            block_mask = M_blk_idxs[:, None] >= N_blk_idxs[None, :]
        else:
            block_mask = M_blk_idxs[:, None] > N_blk_idxs[None, :]
        log_om_beta_ = tl.where(block_mask, log_om_beta_, 0.0)
    else:
        block_mask = None

    if backward:
        neg_log_acc -= tl.sum(log_om_beta_, axis=1)

    p += neg_log_acc[:, None]
    if use_cumsum:
        p += tl.cumsum(log_om_beta_, axis=1, reverse=True)
    else:
        p = tl.dot(log_om_beta_, cm, acc=p, allow_tf32=ALLOW_TF32)
    p = tl.math.exp2(p)
