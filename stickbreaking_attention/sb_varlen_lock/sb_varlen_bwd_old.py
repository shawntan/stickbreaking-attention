...
def _backward(
    DO_ptr,
    stride_doh: tl.constexpr, stride_dom: tl.constexpr, stride_dod: tl.constexpr,
    ...
    DV_ptr, stride_dvh: tl.constexpr, stride_dvn: tl.constexpr,
...
def _backward_one_row(
    ...
    # for i in range(iters - (BLOCK_M // BLOCK_N), iters):
    #     # on_band = (iters - i - 1) < BLOCK_M // BLOCK_N
    #     N_mask = N_blk_idxs < seq_length
    #     NO_N_MASK = (N_blk_idxs_start + BLOCK_N - 1) < seq_length
    #     # --- Recompute block ---
    ...
    #     cumul_att_dA = tl.dot(att_dA.to(cm.dtype), fwd_cm, allow_tf32=ALLOW_TF32) + grad_prev_acc[:, None]
    #     grad_prev_acc += tl.sum(att_dA, axis=1)
    #     beta = 1 - tl.exp2(log_om_beta.to(tl.float32)).to(log_om_beta.dtype)  # 180 -> 175
    #     dqk = att_dA - beta * cumul_att_dA
